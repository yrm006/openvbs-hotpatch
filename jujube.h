...
private:
        ...
        while(p < &m_s.back()){
            ...
            if(pvL->vt == (VT_BYREF|VT_VARIANT)){
                if(pvR->vt == (VT_BYREF|VT_VARIANT)){
                    ...
                    *pvL->pvarVal = *pvR;
                    if(pvL->pvarVal->wReserved1 == VTX_LITERAL){
                        pvL->pvarVal->wReserved1 = VTX_NONE;
                    }
                    pvR->vt = VT_EMPTY;
                }
            }else{
    ...
    bool op_lthan(VARIANT* p){
        ...
        if(
            ( (pvL->wReserved1 == VTX_LITERAL) && (pvL->vt == VT_R8  || pvL->vt == VT_R4  || 
                                                   pvL->vt == VT_I8  || pvL->vt == VT_I4  || pvL->vt == VT_I2  || pvL->vt == VT_I1  ||
                                                   pvL->vt == VT_UI8 || pvL->vt == VT_UI4 || pvL->vt == VT_UI2 || pvL->vt == VT_UI1) ) ||
            ( (pvR->wReserved1 == VTX_LITERAL) && (pvR->vt == VT_R8  || pvR->vt == VT_R4  ||
                                                   pvR->vt == VT_I8  || pvR->vt == VT_I4  || pvR->vt == VT_I2  || pvR->vt == VT_I1  ||
                                                   pvR->vt == VT_UI8 || pvR->vt == VT_UI4 || pvR->vt == VT_UI2 || pvR->vt == VT_UI1) ) ||
        0){
            _variant_t vL, vR;
            if(
        ...
        0){
            _variant_t vL, vR;
            if(
                FAILED(m_err->m_hr = VariantChangeType(&vL, pvL, 0, VT_BSTR)) ||
                FAILED(m_err->m_hr = VariantChangeType(&vR, pvR, 0, VT_BSTR)) ||
            0){
                m_mode = &CProcessor::clock_throw_;
                --m_pc;
                return false;
    ...
    bool op_equal(VARIANT* p){
        //hard number vs hard string
        //    → stringをnumberへ
        //
        //soft number vs hard string
        //    → numberをstringへ
        //
        //hard number vs soft string
        //    → stringをnumberへ
        //
        //soft number vs soft string
        //    → 型変換しない
        //    → numeric Variant < string Variant
        //
        VARIANT* pvL = p-1;
        VARIANT* pvR = p+1;
...
private:
    ...
    bool word_equal(word_t& pc){
        ...
        bool ist = m_s.rbegin()->wReserved1 && (m_s.rbegin()->wReserved1 != VTX_LITERAL);
    ...
    bool word_literal(word_t& pc){
        {
            m_s.push_back( pc.v );
            m_s.back().wReserved1 = VTX_LITERAL;
        }
